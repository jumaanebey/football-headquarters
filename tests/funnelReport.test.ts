...
describe('weekly funnel report', () => {
  ...
  it('keeps seen ≠ confirmed, requested ≠ completed, prompt ≠ backup and export ≠ account apart', () => {
    ...
    expect(text).toContain('saw a result but no confirmed reward: 1');
    expect(text).toContain('export (file left the device) 1 · account (cloud) 1');
  ...
  it('excludes QA traffic by known id and by props.qa, from counts and from cohorts alike', () => {
    ...
    expect(r.returnCohorts).toEqual([{ day: '2026-09-09', newPlayers: 1, d1Observable: 1, d1: 0, d7Observable: 0, d7: 0 }]);
