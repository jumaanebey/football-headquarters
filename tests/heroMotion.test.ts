import {describe,it,expect}from'vitest';import{advanceHeroMotion}from'../game/heroMotion';import{heroPresentation,strideColumn}from'../game/heroPresentation';import{HERO_MOTION_BOUNDS}from'../game/heroMotionBounds';import{firstMatchLesson}from'../game/firstMatchLesson';import sharp from'sharp';
describe('authored hero motion',()=>{
 it('uses a directional brace on contact and returns to idle after the hit',()=>{const idle=advanceHeroMotion(undefined,{x:4,y:4},0,'qb');expect(advanceHeroMotion(idle.state,{x:4,y:4},.05,'qb').frame).toBe(0);const hit=advanceHeroMotion(idle.state,{x:4,y:4,hitFlash:.1},1,'qb');expect(hit.frame).toBe(32);expect(advanceHeroMotion(hit.state,{x:4,y:4,hitFlash:0},1+heroPresentation('qb').stop.seconds+.01,'qb').frame).toBe(0);});
 ...
 for(const key of ['coach','kicker','burner','medic','captain','playmaker','legend']) {
  it(`${key} has four independently authored directions and reactions`,async()=>{
   ...
   for(let direction=0;direction<4;direction++){
    ...
    // Stride frames follow the hero's own contact hold; the frame at a given phase is the profile's, not a fixed table.
    expect(advanceHeroMotion(prior,{x:50,y:50,moving:true,stridePhase:.5},1,key).frame).toBe(direction*9+strideColumn(.5,heroPresentation(key).contactHold));
    ...
    // …and settles to idle once the hero's own settle window has passed.
    expect(advanceHeroMotion(stopped.state,{x:50,y:50},1+heroPresentation(key).stop.seconds+.01,key).frame).toBe(direction*9);
   }
  });
 }
});
