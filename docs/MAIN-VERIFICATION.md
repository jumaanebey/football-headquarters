...
## Follow-up after PR #60 (2026-09-10, main `02091e4`)

**Provenance correction accepted.** The earlier "kill switch 13/13" line in this document described a worker that did **not** contain the claim-before-navigation correction, so it did not prove the original main worker. PR #60 ported that correction. The rehearsals below were rerun natively against `02091e4`, which does contain it.

| Rehearsal | Result on `02091e4` |
| --- | --- |
| Kill switch (`scripts/pwa-killswitch-check.mjs`, built preview on a dedicated port) | **13/13** — every `fhq-*` cache deleted, worker unregistered, the open page re-navigated exactly once with no loop, club data and a foreign cache untouched, club-server and auth never cached by either worker, normal worker re-registers and controls |
| Two-version tabs (`scripts/pwa-multitab-check.mjs`) | **verified** — the previous version's caches survive activation while an old tab is open; that tab's cached campus sheet and lazy editor chunk still load after the deploy removed them; an uncached removed asset fails cleanly; the old tab is not reloaded by the other tab's activation and reloads into the new bundle on request. Host limitation recorded in the output: nothing can serve a file the tab never requested and the host no longer ships |
| Journey and lifecycle (`scripts/journey-check.mjs`, 2–3 rounds) | **verified** — ten of ten assertions per round, four different heroes drawn, a signature used, transfer 23.5 MB cold → 0.03 MB warm, decoded art entries 5 → 5, subscribers 0, canvases 14 → 14, DOM +2 nodes, no console errors |

**Defect found in my own tooling, fixed here.** With #60's hardened `tap` (a failed click no longer counts as success) the journey reported "Scouting reachable from Roster" as a failure on three rounds. It is **not** a product regression: the accessible-name selector matched three buttons and `.first()` was the campus *Scouting Dept* building sitting behind the roster modal, so the click was intercepted by the overlay while the real Scout control was the third match. The check now scopes the tap to the open dialog and additionally requires the scouting view to appear, so "reachable" means the player-visible control worked, not that some button existed. All rounds pass with the stronger assertion.

**Connection assessment ported from PR #55.** `pwa/connection.ts` now separates the failure shapes the player must act on differently: browser offline, transport failure despite a claimed connection, expired sign-in, and a reached-but-unavailable server, each with its own summary and an `action` of `wait`, `sign-in` or none. It adds `lastOkAt`, `lastFailureAt`, `consecutiveFailures`, the owning `account`, `resetConnectionForAccount(next)` for sign-in/sign-out/link, and a teardown function from `armConnectionTracking`. Existing consumers are unaffected: `ClubConnectionControl` keeps reading `summary` and `online`, and `game/online/authorityClient.ts` keeps calling `reportClubServer` with the statuses it already reports.

Two call sites remain for Codex, neither of which this change makes: call `resetConnectionForAccount(nextOwner)` where the signed-in account changes, and surface `'unavailable'` by mapping a 5xx answer in the authority transport (today every transport failure reports `'offline'`, which reads as *transport-failure* — accurate, just less specific).
