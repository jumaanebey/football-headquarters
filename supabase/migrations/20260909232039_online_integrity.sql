-- Football Headquarters online delivery and save integrity.
-- Reviewed against restored project ruzkpbvgzvqrrnexrffz on 2026-09-09.
-- Existing data (4 bases, 1 report, 3 saves) is retained without backfill or truncation.
-- This migration protects delivery/concurrency, not battle score authority.
...
drop policy if exists "fhq_saves_own" on public.fhq_saves;
...
-- Settings already offers removal of the player's own published base. Previously
-- no DELETE policy existed, so PostgREST could return success after deleting zero.
grant delete on public.fhq_bases to authenticated;
drop policy if exists "bases: own delete" on public.fhq_bases;
create policy "bases: own delete" on public.fhq_bases for delete to authenticated
  using (pid = (select auth.uid())::text);
