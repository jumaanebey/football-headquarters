import { laneClear } from './raidNavigation';
import type { BTroop, BBuilding } from '../../battle';
import { nearestTroop } from '../../battle';
import type { BattleEngine } from './engine';
import { EQUIPMENT_COUNTERS, TACTICAL_WARNINGS, counterRole, defenseResistance, displaceFrom, splashFalloff } from './defenseCounters';

type State=BattleEngine['state'];
/** Fixed-tick warnings, locked impact areas, and role-aware equipment outcomes. */
export function stepCounterEquipment(b:BBuilding,s:State,dt:number,hit:(t:BTroop,damage:number)=>void,tactical=false) {
  const flavor=b.flavor??'jugs',rule=EQUIPMENT_COUNTERS[flavor];
  const windup=tactical?TACTICAL_WARNINGS[flavor]:rule.windup;
  // Longer readable warnings preserve sustained pressure; a 5% raid rebalance
  // keeps exposed rushes costly now that orders and solid cover can avoid attacks.
  ...
  if(!attack){
    ...
    const target=nearestTroop(b.x,b.y,candidates,b.range!);
    if(!target)return;
    const signature=(b.level??0)>=10&&(b.counterSignatureT??1)<=0;
    attack=b.counterAttack={targetId:target.id,x:target.x,y:target.y,remaining:windup,signature};
    if(tactical)s.shots.push({sx:b.x,sy:b.y,tx:target.x,ty:target.y,t:0,dur:windup,rot:0,flavor});
    if(windup){
      s.pulses.push({x:attack.x,y:attack.y,r:rule.radius||3,life:windup,maxLife:windup,color:'#fef08a'});
      s.fx.push({type:'yards',text:`${signature?'POWER ':''}${rule.name.toUpperCase()} →`,x:b.x,y:b.y-4,life:windup,maxLife:windup,color:'#fef08a'});
      return;
    }
  }
  attack.remaining-=dt;
  ...
  if(flavor==='cooler') {
    ...
    for(const t of s.troops){if(t.dead)continue;const falloff=splashFalloff(Math.hypot(t.x-attack.x,t.y-attack.y),radius);if(!falloff)continue;
      hit(t,damage*rule.damage*falloff*(signature?1.4:1));t.braceT=Math.max(t.braceT??0,rule.duration*falloff);t.lastDefenseEffect='Shirt entanglement';}
    s.pulses.push({x:attack.x,y:attack.y,r:radius,life:.35,maxLife:.35,color:'#f472b6'});
  } else if(target) {
    const resist=defenseResistance(target);
    if(flavor==='sled'){
      hit(target,damage*rule.damage*(.65+.35*resist.brace)*(signature?1.4:1));
      ...
      for(const t of targets){hit(t,damage*rule.damage);t.flagT=Math.max(t.flagT??0,rule.duration*defenseResistance(t).discipline);t.lastDefenseEffect='Flag: movement and attack disrupted';}
    } else {
  ...
  if(!tactical)s.shots.push({sx:b.x,sy:b.y,tx:rule.radius?attack.x:target?.x??attack.x,ty:rule.radius?attack.y:target?.y??attack.y,t:0,dur:.3,rot:0,flavor});
}
