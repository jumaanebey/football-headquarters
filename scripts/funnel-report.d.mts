...
export interface FunnelReport { ignoredRows: number; window: { from: string; to: string; days: number }; data: string; denominator: number; events: number; qa: { rows: number; players: number; marker: string }; steps: FunnelStepCount[]; returnCohorts: FunnelCohort[]; returningInWindow: number }
...
export function fetchRows(url:string,key:string,since:string,until?:string):Promise<unknown[]>;
