...
for (let round = 1; round <= rounds; round++) {
  ...
  line(/Roster|Game Day/.test(await text()), `round ${round}: club named and campus reached`);
  ...
  if (field) {
    ...
    line(drawn.size >= 2, `round ${round}: ${drawn.size} different heroes drawn on the field (${[...drawn].join(', ')})`);
    ...
    line(sig, `round ${round}: a signature became ready and was used (beats seen: ${beat.join(',') || 'none captured at sample time'})`);
    ...
    line(result, `round ${round}: result screen reached`);
  ...
  line(back, `round ${round}: returned to the campus`);
