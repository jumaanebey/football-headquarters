...
export const FUNNEL = [
  ...
  ['first_kickoff', 'Kicked off a first game', []],
...
export function computeFunnel(input, { now = Date.now(), days = 7, qaPids = new Set() } = {}) {
  ...
  const sanitized = sanitizeRows(input);
  const rows = sanitized.rows.filter(r => r.t <= now), malformed = sanitized.malformed;
  ...
  const cohorts = [];
  for (const [pid, t] of firstSeen) if (t >= since && t <= now) { const d0 = Math.floor(t / DAY); const days_ = activeDays.get(pid); cohorts.push({ pid, day: new Date(d0 * DAY).toISOString().slice(0, 10), d1: today > d0 + 1 ? days_.has(d0 + 1) : null, d7: today > d0 + 7 ? days_.has(d0 + 7) : null }); }
  const byDay = {};
  ...
  return {
    ...
    returningInWindow: [...players].filter(pid => [...(activeDays.get(pid) ?? [])].some(day => day > Math.floor(firstSeen.get(pid) / DAY) && day >= Math.floor(since / DAY))).length,
  };
}
...
export function renderFunnel(report) {
  ...
  lines.push('Independent milestone counts, not ordered conversion rates. Missing historical events are not proof of abandonment.');
  lines.push('Returns require activity on distinct UTC days; a saved club alone is not a return. QA exclusions are limited to documented IDs and explicit markers.');
  ...
  lines.push(`  result recorded without a reward event in this window: ${d.resultsNotConfirmed} · upgrade request without a completion event: ${d.upgradesRequestedNotCompleted} · backup prompt without a completion event: ${d.backupPromptNotCompleted}`);
  lines.push(`  backups by method: export (download requested) ${d.backupMethods.export} · account (cloud) ${d.backupMethods.account}${d.backupUnknownMethod ? ` · unknown method ${d.backupUnknownMethod}` : ''}`);
  lines.push(`  returning players in window: ${report.returningInWindow}`);
  lines.push('  return cohorts (first seen in available history, not lifetime-new; completed UTC days only)');
...
export async function fetchRows(url, key, since, { fetchImpl = fetch, page = 1000, until = new Date().toISOString() } = {}) {
  const rows = [];
  for (let offset = 0; ; offset += page) {
    const res = await fetchImpl(`${url}/rest/v1/fhq_events?select=pid,event,props,ts&ts=gte.${encodeURIComponent(since)}&ts=lte.${encodeURIComponent(until)}&order=ts.asc,id.asc&limit=${page}&offset=${offset}`, { headers: { apikey: key, Authorization: `Bearer ${key}` } });
...
const main = async () => {
  ...
  if (!Number.isFinite(days) || days <= 0 || days > 365 || !Number.isFinite(now)) { console.error('invalid --days or --now'); process.exit(1); }
  ...
  if (fixture) { rows = JSON.parse(readFileSync(fixture, 'utf8')); source = `fixture ${fixture}`; sourceKind = 'fixture'; }
    ...
    rows = await fetchRows(url, key, from, {until:new Date(now).toISOString()});
