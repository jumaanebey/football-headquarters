...
const inventory=JSON.parse(await readFile('scripts/qa-accounts.json','utf8'));inventory.accounts.push({id:session.user.id,purpose:'Connected Lineup QA · v9 integration evidence; retained for deferred cleanup'});await writeFile('scripts/qa-accounts.json',JSON.stringify(inventory,null,2)+'\n');
...
answer=await call(op({type:'club.rename',name:'Connected Lineup QA'}));check(answer.ok,'named QA club');
