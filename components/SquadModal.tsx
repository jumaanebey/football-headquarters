...
interface Props{
 ...
 onOpenPractice?:()=>void;onOpenWeightRoom:(playerId?:string)=>void;onOpenFacility?:(type:BuildingType)=>void;
}
 ...
 return <Sheet title="Your roster" subtitle={`${roster.length} teammates · ${mean} average Power`} onClose={onClose} maxWidth="max-w-6xl">
  <div className="fhq-roster-workspace">
   <nav className="fhq-roster-destinations" aria-label="Player development rooms">
    ...
    <button onClick={onOpenPractice}>Practice Field<small>Routes & timing</small></button>
   ...
   <p className="fhq-roster-explainer">Power includes position, rarity and training. Weight Room grows Strength and level; Practice grows Speed; Film grows IQ.</p>
   ...
   <div className="fhq-roster-grid">{shown.map(p=>{
    ...
    return <article key={p.id} className="fhq-roster-card" aria-label={`${p.name}, ${p.role}, level ${p.level}`}>
     ...
     <div className="fhq-roster-identity"><p>{p.rarity} · Level {p.level}</p><h3>{p.name}</h3><button className="fhq-roster-location" onClick={()=>activity.practice?onOpenPractice?.():activity.building===BuildingType.TRAINING_PITCH?onOpenWeightRoom(p.id):onOpenFacility?.(activity.building)}>{activity.label} →</button></div>
     <div className="fhq-roster-power"><strong>{Math.round(unitPower(p))}</strong><small>POWER</small></div>
     <dl className="fhq-roster-stats">{[['Strength',p.stats.strength],['Speed',p.stats.speed],['IQ',p.stats.iq]].map(([name,value])=><div key={name}><dt>{name}</dt><dd>{value} <span>{name==='Strength'?'Gym':name==='Speed'?'Practice':'Film'}</span></dd></div>)}</dl>
     <p className="fhq-roster-trait">{trait?`${trait.label} · ${trait.desc}`:'Keep training to strengthen this position.'}</p>
     <div className="fhq-roster-card-actions"><button onClick={()=>onOpenWeightRoom(p.id)}>{growth.training.kind==='training'?(growth.training.collectable?'Collect growth':'View workout'):`Train ${p.name}`}<small>{growth.training.kind==='training'?'Current group session':`Gym: Strength +2 · Level ${p.level+1}`}</small></button>
