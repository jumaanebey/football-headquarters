...
export const SquadModal: React.FC<Props> = ({ roster, resources, club, blocked, playerFilter, onFilterChange: setPlayerFilter, onCollect, onClose, onTrainGroup, onCutPlayer, onOpenHeroes, onScout }) => {
    ...
    >
      ...
      {view === 'players' ? <div className="p-4 space-y-4">
            ...
            {drills.map(drill => {
              ...
              return (
                <button
                  ...
                  disabled={blocked || trainingBusy || !canAfford}
                ...
                >
                  ...
                  <div className={`shrink-0 flex items-center gap-1 px-3.5 py-2.5 rounded-xl font-bold text-sm ${canAfford ? 'bg-sky-600 text-white' : 'bg-slate-800 text-slate-500'}`}>
                    <Play size={14} fill="currentColor" /> {trainingBusy ? 'Busy' : canAfford ? 'Train' : 'Low ⚡'}
                  </div>
                </button>
              );
            })}
        ...
        {(() => {
          ...
          return (
            <button onClick={() => onTrainGroup(UnitGroup.OFFENSE_LINE, 'scrimmage')} disabled={blocked || trainingBusy || !canAfford}
              className={`w-full flex items-center justify-between gap-2 p-3 rounded-2xl border-2 transition-all text-left active:scale-[0.98]
              ...
              <div className={`shrink-0 flex items-center gap-1 px-3.5 py-2.5 rounded-xl font-bold text-sm ${canAfford ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-500'}`}>
                <Play size={14} fill="currentColor" /> {trainingBusy ? 'Busy' : canAfford ? 'Run it' : 'Low ⚡'}
              </div>
            </button>
          );
        })()}
