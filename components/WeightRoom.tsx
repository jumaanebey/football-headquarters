import {FacilityUpgrade,FacilityGoals} from './FacilityUpgrade';
...
export function WeightRoom({club,blocked,initialPlayer,onClose,onStart,onCollect,onUpgrade,onGameDay,onRoster,onOpenFacility,onAction,onPractice}:{onAction?:(a:ClubAction)=>void;onPractice?:()=>void;club:GameState;blocked:boolean;initialPlayer?:string;onClose:()=>void;onStart:(unit:GameState['roster'][number]['unit'],id:string)=>void;onCollect:(b:BuildingInstance)=>void;onUpgrade:(id:string,cost:number)=>void;onGameDay:()=>void;onRoster?:()=>void;onOpenFacility?:(type:BuildingType)=>void}){
 const [selected,setSelected]=useState(initialPlayer??club.roster[0]?.id);
 const [previewUnit,setPreviewUnit]=useState<GameState['roster'][number]['unit']|'ALL'>(club.roster.find(p=>p.id===initialPlayer)?.unit??club.roster[0]?.unit??'ALL');const[now,setNow]=useState(Date.now());
 ...
 const participants=scheduled?(scheduled.station==='weights'?scheduledPlayers(club):[]):club.roster.filter(p=>active?(active.targetUnit==='ALL'||p.unit===building.targetUnit):previewUnit==='ALL'||p.unit===previewUnit);
 ...
 <div className="fhq-weight-room-sign"><small>{working?`${participants.length} teammates in this session`:ready?'Session complete · collect your growth':`${previewUnit==='ALL'?'Whole team':indoorGroupName[previewUnit]} · ${participants.length} of ${club.roster.length} teammates`}</small></div>
 ...
 {!active&&<DevelopmentPanel afterAction={<FacilityUpgrade club={club} building={building} blocked={blocked} onUpgrade={onUpgrade}/>} key={player.unit} initialUnit={previewUnit} focusedPlayer={player.name} onUnitChange={setPreviewUnit} club={club} station="weights" blocked={blocked||!onAction} onAction={a=>onAction?.(a)}/>}
 ...
 {active&&<FacilityUpgrade club={club} building={building} blocked={blocked} onUpgrade={onUpgrade}/>}
 <nav className="fhq-player-room-nav"><button onClick={onPractice}>Practice Field</button></nav>


 </section><FacilityGoals building={building}/></div></Sheet>
}
