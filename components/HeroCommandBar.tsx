...
export function HeroCommandBar({ heroes, troops, selected, onSelect, onAbility }: {
  ...
  return <div role="group" aria-label="Hero commands" className="grid gap-2 mb-2" style={{ gridTemplateColumns: `repeat(${Math.min(5, heroes.length)}, minmax(0, 1fr))` }}>
    {heroes.map(hero => {
      ...
      const status = !actor ? 'Select · deploy' : actor.dead ? 'Out of play' : actor.activeAction ? 'In motion' : cooldown > 0 ? `Ready in ${cooldown}s` : 'Signature ready';
      return <button key={hero.key} type="button" disabled={disabled} aria-label={`${hero.name}: ${!actor ? 'select for deployment' : hero.abilityName}. ${status}`}
        ...
        <span className="min-w-0 leading-tight">
          ...
          {actor && <span className="block text-[11px] mt-0.5">{status}</span>}
        </span>
      </button>;
    })}
  </div>;
