...
export function DesktopClubPanel({ state, onClose, onGames, onHeroes, onDefense, onRanks, onShare }: { state: GameState; onClose: () => void; onGames: () => void; onHeroes: () => void; onDefense: () => void; onRanks: () => void; onShare: () => void }) {
 ...
 return <aside aria-label="Club command center" className="fhq-desktop-club space-y-4 border-l border-slate-700 bg-slate-950/95 p-5">
   <button onClick={onClose} className="min-h-11 rounded-lg border border-slate-600 px-3 text-sm text-white">Close command center</button><div><p className="text-xs uppercase tracking-widest text-orange-300">Club command center</p><h2 className="mt-2 text-xl font-bold text-white">{state.teamName}</h2></div>
