...
export function EquipmentDetails({club,slotId}:{club:GameState;slotId:string}) {
  ...
  return <details className="mt-3 rounded-xl border border-slate-700 p-3 text-sm text-slate-300">
    ...
    {model.current && <p className="mt-2">Equipment Grit {Math.round(model.current.durability)}{model.next ? ` → ${Math.round(model.next.stats.durability)}` : ''} · Base pressure {Math.round(model.current.damage)}{model.next ? ` → ${Math.round(model.next.stats.damage)}` : ''}{model.next ? ` at level ${model.next.toLevel}` : ' at maximum level'}. Before squad and formation bonuses.</p>}
    {model.currentBoosted && <p className="mt-2">With your current squad and formation: {Math.round(model.currentBoosted.durability)} Grit · {Math.round(model.currentBoosted.damage)} base pressure.</p>}
