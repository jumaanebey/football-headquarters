...
export function MatchScoutBoard({buildings}:{buildings:BattleBuildingDef[]}){
 ...
 {[...buildings].sort((a,b)=>a.x+a.y-b.x-b.y).map(b=>{const p=point(b.x,b.y);const size=b.kind==='hq'?100:b.kind==='wall'?9:60;return <g key={b.id}><title>{b.kind==='hq'?'Stadium':b.kind==='defense'?`${b.flavor??hashDefenseFlavor(b.id)??'jugs'} defense`:b.kind==='wall'?'Blocking wall':'Facility'}</title>{b.kind==='wall'?<rect x={p.x-4} y={p.y-3} width="8" height="6" rx="1" fill="#9ca3af"/>:<><foreignObject x={p.x-size/2} y={p.y-size*.8} width={size} height={size}><ScenerySprite src={b.art??(b.kind==='defense'?defenseSprite(b.flavor??hashDefenseFlavor(b.id)??'jugs',b.level??1):buildingSprite(b.kind==='hq'?BuildingType.STADIUM:BuildingType.TRAINING_PITCH,b.level??1))} style={{width:'100%',height:'100%',objectFit:'contain'}}/></foreignObject>{b.kind==='hq'&&<text x={p.x} y={p.y+14} textAnchor="middle" fill="#fde68a" fontSize="10" fontWeight="bold">STADIUM</text>}{b.kind==='defense'&&<circle cx={p.x} cy={p.y+5} r="3" fill="#fb7185"/>}</>}</g>})}
