...
export function BattleHeroSprite({ heroKey, actor, fighting, simulationTime, filter, kit }: {
  kit?: TeamKit; heroKey: string; actor: Pick<BTroop, 'moving' | 'actionPoseT' | 'abilityPoseT' | 'face' | 'strideSeconds' | 'stridePhase' | 'signatureFrame' | 'truckT' | 'x' | 'y' | 'hitFlash'>; fighting: boolean; simulationTime: number; filter?: string;
}) {
  ...
  return <>
    <SpriteFrames kit={kit} sources={sources} duration={actor.strideSeconds} style={{ filter, transform: (actor.face ?? 1) > 0 ? 'scaleX(-1)' : undefined }} />
    {hasModernHero(heroKey) && <AnimatedHero kit={kit} tier="battle" clockSeconds={simulationTime} motionFrame={HERO_MOTION_BOUNDS[heroKey] && mode !== 'attack' && mode !== 'signature' ? sample.frame : undefined} heroKey={heroKey} mode={mode} facing={actor.face} cycle={actor.strideSeconds} loadSignatureArt
