...
export const ROLL_COST_GEMS = 20; // balance-sim: ~2-3 rolls/day F2P
...
export const rollHero = (heroes: HeroState[]): RollResult => {
  ...
  return owned
    ? { key: def.key, name: def.name, isNew: false, shards: 14 + Math.floor(Math.random() * 9) } // 14–22 (sim-tuned 5★ pace)
