...
import { HERO_PRESENTATION } from '../game/heroPresentation';
import { openCampusArt } from '../game/artGate';
import { HERO_DEFS } from '../battle';
import { spriteMotion } from '../game/spriteMotion';

const params = new URLSearchParams(location.search);
if (params.get('profiles') === 'off') for (const key of Object.keys(HERO_PRESENTATION)) delete HERO_PRESENTATION[key];
openCampusArt();
...
function Fixture() {
  ...
  return <>
    ...
    {MODERN_HEROES.map(key => { const a = actors.current[key]; const def = HERO_DEFS.find(h => h.key === key)!; return <div className="row" data-hero={key} key={key}>
      ...
      {(scale === 'both' || scale === 'phone') && <span className="cell phone"><img src={def.art} alt="" /><BattleHeroSprite heroKey={key} actor={a} fighting simSeconds={t} /></span>}
      {(scale === 'both' || scale === 'large') && <span className="cell large"><img src={def.art} alt="" /><BattleHeroSprite heroKey={key} actor={a} fighting simSeconds={t} /></span>}
